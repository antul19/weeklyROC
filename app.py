...
COLORS = {
    ...
    # Event Zone Colors
    "crisis_zone": "rgba(255, 68, 68, 0.12)",   # Faint Red
    "war_zone":    "rgba(255, 165, 0, 0.15)"    # Faint Amber/Orange
}
...
def make_rebased_macro_chart(data_dict: dict, events_list: list, zone_color: str, title: str) -> go.Figure:
    ...
    # Add dynamically selected event zones (Crises vs Wars)
    for ev in events_list:
        fig.add_vrect(
            x0=ev["start"], x1=ev["end"],
            fillcolor=zone_color, opacity=0.8, layer="below", line_width=0,
            annotation_text=ev["name"], annotation_position="top left",
            annotation_font=dict(family="IBM Plex Mono", size=10, color="#FFFFFF"),
            annotation_textangle=-90
        )

    layout = _base_layout(title, height=450) # Slightly shorter so both fit easily on screen
    layout["margin"]["t"] = 60
    layout["yaxis"]["type"] = "log"
    layout["yaxis"]["title"] = "Index/Asset Value (Log Scale)"
    ...
    with st.spinner("Loading Global Macro Data..."):
        ...
        if global_data:
            ...
            for i, asset_name in enumerate(asset_names):
                # Default to checking US, Gold, and Oil on load to show off the 1970s
                default_val = True if "US" in asset_name or "Gold" in asset_name or "Crude" in asset_name else False
                with cols[i % 4]:
            ...
            if selected_assets:
                ...
                # Extract the dynamic start year for the title
                df_combined = pd.DataFrame(filtered_data).dropna()
                start_yr_label = df_combined.index[0].year if not df_combined.empty else "Selected Assets"

                st.plotly_chart(
                    make_rebased_macro_chart(filtered_data, FINANCIAL_CRISES, COLORS["crisis_zone"], f"Financial Liquidity Crises (Rebased to 100 in {start_yr_label})"), 
                    use_container_width=True, config={"displayModeBar": False}
                )
                
                st.plotly_chart(
                    make_rebased_macro_chart(filtered_data, GEOPOLITICAL_WARS, COLORS["war_zone"], f"Geopolitical Conflicts (Rebased to 100 in {start_yr_label})"), 
                    use_container_width=True, config={"displayModeBar": False}
                )
            else:
